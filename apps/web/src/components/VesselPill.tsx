...
export function VesselPill({ vesselName, hasVessels, onClick }: Props) {
  ...
  //   1. vesselName present  → vessel name in teal
  //   2. user has vessels    → "No vessel"      (short form — D6.78)
  //   3. user has no vessels → "Select vessel"  (call-to-action)
  //
  // Sprint D6.78 — Karynn UX: shortened both empty-state labels to keep
  // the bottom row (vessel pill + verbosity dropdown + Log pill) on a
  // single line for default mobile chats. The previous "No vessel — tap
  // to switch" / "General mode — add a vessel" wrapped on narrow screens
  // because they were ~25-30 chars wide. The wrap on long vessel names
  // is acceptable; the empty-default wrap is not.
  //
  // Also dropped the px-3 py-1.5 wrapper div — its padding stacked with
  // the bottom-row gap from the parent layout and pushed the verbosity
  // dropdown / Log pill toward the right edge, contributing to wrap.
  const emptyLabel = hasVessels ? 'No vessel' : 'Select vessel'
