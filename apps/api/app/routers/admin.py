...
class AdminStats(BaseModel):
    ...
    # the user (complete_miss + partial_miss in hedge_audits — moved
    # off tier_router_shadow_log when tier_router was killed D6.97).
...
async def get_stats(
    ...
    async with pool.acquire() as conn:
        ...
        # D6.97 — "real" hedge rate moved from tier_router_shadow_log
        # to hedge_audits. The shadow log stopped receiving new rows
        # when the tier_router was killed; hedge_audits writes on
...
# D6.97 -- tier_router shadow log admin endpoints removed alongside
# the engine-side kill. tier_router_shadow_log table is preserved
# (migration 0093) for 90-day forensics; query via psql if needed.
# See docs/sprint-audits/tier-router-shadow-kill-2026-05-19.md for
# the full rationale.
