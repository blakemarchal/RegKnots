...
6. **Corpus gap fills (prioritized).** Detailed analysis at [`docs/corpus-gap-report-2026-05-09.md`](corpus-gap-report-2026-05-09.md). Top targets in effort/impact order:
   - **NVIC backfill** (~1 hr): we have 158 NVICs but the 2005 series has gaps (04-05/06/07 missing). USCG NVIC index page is public; existing `nvic` adapter handles it.
   - **Load Lines Convention re-ingest** (~3 hr): currently 4 chunks; should be 100-200. Foundational stability + freeboard reference. Source freely available.
   - **IMO HSC/IGC/IBC chapter granularity** (~6-8 hr): each code is in the corpus (~300 chunks each) but stored under a single `section_number` so chapter-level citations can't be verified. Re-chunk with chapter-precise IDs.
   - **FSS Code + LSA Code ingest** (~10 hr combined): both entirely missing despite heavy SOLAS Ch.II-2 / Ch.III references. Source acquisition needed (IMO Publishing). Highest user impact for fire-system and lifesaving-equipment questions.
