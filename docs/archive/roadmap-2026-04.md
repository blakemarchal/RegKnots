> **ARCHIVED 2026-05-08 — see /docs/roadmap.md for current state.**
...
### Sprint D5.5 — top-2-chunks-per-section retrieval dedup (2026-04-24)
Fixed the retriever bug uncovered in Karynn's 2026-04-23 evening session: the section-level deduplicator kept only the highest-similarity chunk per section, suppressing specific-answer chunks (like Table 1 of 46 CFR 199.175 containing lifeboat flare counts) behind intro-chunk siblings. Changed `_MAX_CHUNKS_PER_SECTION` from 1 to 2 in both single-group and multi-group dedup paths; hybrid merge (_merge_chunks) respects the cap by boosting the weakest sibling when an identifier/keyword hit targets an at-cap section.

Results: **overall hedge rate 23.0% → 19.1%** (biggest single-sprint drop in the D-series). Sailor-speak hedge rate 25.6% → 22.2%. A-or-A− went 96.7% → 96.1% (one extra F grade, all 6 Fs are citation-format hallucinations unrelated to the dedup change). Smoke test on "how many parachute flares does my lifeboat need?" confirms chunk 10 of 46 CFR 199.175 (Table 1 with counts) now surfaces in top-8. Smoke test on Karynn's exact conversational phrasing ("So you can't tell me how many...") still fails to pull 46 CFR 199.175 because the phrasing has weak vector similarity to the table content — a query-phrasing issue, not a dedup issue. Fix-on-fix candidate: add router escalation to Opus when query style suggests "count/quantity lookup on a specific vessel."

Also fixed: D2-LOG hedge detection was missing from `chat_with_progress` (SSE streaming path used by real users). Only `chat()` (eval path) had the hook. That's why `retrieval_misses` was empty despite Karynn's visible hedges. Post-fix, all real-user hedges get logged.

### Sprint D5.4 — WHO IHR (2005) ingest (2026-04-23)
Ingested WHO International Health Regulations (2005, as amended 2014/2022/2024) from the consolidated PDF on apps.who.int — 75 sections, 163 chunks. Covers port health + Ship Sanitation Control Certificate (Annex 3 — the "deratting certificate"), closing Karynn's 2026-04-22 hedge on sanitary inspection certificates. Both canaries (W-1 SSCC renewal, W-2 port health measures on arrival) graded A. Naturalistic eval subset reached 100% A-or-A− for the first time (was 96.6%). Small sailor-speak regression (100% → 95.6%) due to 3 new citation hallucinations on queries unrelated to port health — flagged as citation-verifier variance rather than D5.4-caused. `who_ihr` tagged Tier 1 authority + its own `who` source group in the retriever.
